...
export function filterPublish<E extends { data: { publish?: boolean; draft?: boolean } }>(
  e: E,
): boolean {
  return e.data.publish !== false && e.data.draft !== true;
}
...
export function entryUrl<K extends ColKey>(
  key: K,
  entry: { slug?: string | null | undefined } | CollectionEntry<K>,
): string {
  const raw = entry?.slug ?? '';
  const slug = String(raw).replace(/\/index$/, '');
